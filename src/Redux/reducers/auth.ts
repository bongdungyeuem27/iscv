...
export const refreshEmployee = createAsyncThunk<
  {
    employee: IEmployee | undefined
  },
  {
  ...
  { dispatch: AppDispatch; state: RootState }
>('auth/refresh_employee', async () => {
  const employee = await getEmployeeByUser({ user: store.getState().auth.account! })
    .then((success) => success.data.data.employeeByUser)
    .catch((error) => console.log(error))
  if (!employee) return { employee: undefined }
  else {
...
export const authReducer = createSlice({
  ...
  extraReducers(builder) {
    ...
    builder.addCase(refreshEmployee.fulfilled, (state, action) => {
      if (action.payload) {
        state.employee = action.payload.employee
        loading.close()
      }
